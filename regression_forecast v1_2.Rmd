...
future_x<- future_x%>% group_by(year)%>% 
  summarise(num_ev= sum(num_ev), gas_price = mean(gas_price), m_p_k= mean(m_p_k), dollar.per.kw = mean(dollar.per.kw), population=sum(population), avg_income = mean(avg_income),  chargers= sum(chargers), avg_car_price = mean(avg_car_price))
